...
export async function init(h = defaultHelper) {
  ...
  let replacedConfig = rftoolsConfig.replace(
    /^(livingmatter\s*\{\s*)\n([^}]*)(\n\s*\})$/m,
    (m, p1, p2, p3) => {
      const vanillaLM = p2
        .split(/\s*S:living./)
  ...
  replacedConfig = replacedConfig.replace(
    /^(mobspawnamounts\s*\{\s*)([^}]*)(\n\s*\})$/m,
    (m, p1, p2, p3) => {
      const matchesChunks = _.chunk(
        [...p2.matchAll(/^\s*S:(.+)\.spawnamount\.([0-2])"?\s*<([^>]+)>/gm)],
        3
      )
        .filter((chunk) => {
  ...
  h.result(
    `Rewritten ${mobspawnamounts.length} entities and ${livingmatter.length} livingmatter`
  )
}
